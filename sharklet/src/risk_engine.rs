...
impl Default for RiskConfig {
    fn default() -> Self {
        Self {
            max_daily_loss_usd: env_or("RISK_MAX_DAILY_LOSS_USD", 150.0),
            max_hourly_loss_usd: env_or("RISK_MAX_HOURLY_LOSS_USD", 50.0),
            max_trade_size_usd: env_or("RISK_MAX_TRADE_SIZE_USD", 15_000.0),
            max_flash_loan_usd: env_or("RISK_MAX_FLASH_LOAN_USD", 15_000.0),
            max_gas_spent_today_usd: env_or("RISK_MAX_GAS_SPENT_TODAY_USD", 30.0),
            max_consecutive_failures: env_or("RISK_MAX_CONSECUTIVE_FAILURES", 6),
            wallet_min_balance_usd: env_or("RISK_WALLET_MIN_BALANCE_USD", 5.0),
            min_rpc_confidence: env_or("RISK_MIN_RPC_CONFIDENCE", 0.78),
            circuit_breaker_pause_secs: env_or("RISK_CB_PAUSE_SECS", 90),
            failure_reset_window_secs: env_or("RISK_FAILURE_RESET_SECS", 3_600),
            adaptive_limits: env_or("RISK_ADAPTIVE_LIMITS", true),
            // (wallet_balance_ceiling_usd, max_trade_usd, max_flash_loan_usd, max_gas_today_usd)
            // Checked in order - first tier whose ceiling >= wallet balance wins.
            // Scales position size up automatically as the wallet grows from trading profit.
            adaptive_tiers: vec![
                (100.0,      500.0,     500.0,     5.0),   // starter capital - stay small, prove the strategy works
                (500.0,      2_500.0,   2_500.0,   10.0),  // building confidence
                (2_000.0,    8_000.0,   8_000.0,   20.0),  // established
                (10_000.0,   20_000.0,  20_000.0,  35.0),  // scaling
                (50_000.0,   40_000.0,  40_000.0,  60.0),  // aggressive
                // above 50k wallet balance: falls through to max_trade_size_usd / max_flash_loan_usd / max_gas_spent_today_usd
            ],
        }
    }
}
...
impl RiskEngine {
    ...
    pub fn from_toml(path: &str) -> Self {
        let cfg = std::fs::read_to_string(path)
            .ok()
            .and_then(|c| toml::from_str::<RiskConfig>(&c).ok())
            .unwrap_or_default();
        Self::new(cfg)
    }
    ...
    pub fn evaluate(
        ...
        if trade_size_usd > max_trade {
            return RiskDecision::Reject(format!("trade size ${:.0} > max ${:.0}", trade_size_usd, max_trade));
        }
        if flash_loan_usd > max_loan {
            return RiskDecision::Reject(format!("flash loan ${:.0} > max ${:.0}", flash_loan_usd, max_loan));
        }
        ...
        if hourly >= self.limits.max_hourly_loss_usd {
            return RiskDecision::Reject(format!(
                "hourly loss ${:.2} >= limit ${:.2}",
                hourly, self.limits.max_hourly_loss_usd
            ));
        }
        ...
        if daily >= self.limits.max_daily_loss_usd {
            return RiskDecision::Reject(format!(
                ...
                daily, self.limits.max_daily_loss_usd
            ));
        }

        let gas_today = self.gas_spent.sum_within(86_400);
        if gas_today >= max_gas_today {
            return RiskDecision::Reject(format!("daily gas ${:.2} >= limit ${:.2}", gas_today, max_gas_today));
        }
