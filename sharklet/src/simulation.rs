// src/simulation.rs – Full enterprise, no duplicates
use ethers::prelude::*;
...
impl<M: Middleware + 'static> Simulator<M> {
        ...
        if passed {
            ...
            if let Some(profit_usd) = estimated_profit_after_gas_usd {
                if profit_usd < self.config.min_profit_after_gas_usd {
                    trace.push((
                        "profit floor".into(),
                        CheckStatus::Fail(format!("profit after gas ${:.2} < min ${:.2}", profit_usd, self.config.min_profit_after_gas_usd)),
                    ));
                    passed = false;
                }
            }
    ...
    async fn deep_simulate(&self, sized: &SizedTrade, trace: &mut Vec<(String, CheckStatus)>) -> (Option<U256>, Option<U256>, Option<f64>) {
        ...
        let profit_after_gas = sized.net_profit_usd; // using net_profit_usd
