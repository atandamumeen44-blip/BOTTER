...
impl<M: Middleware + 'static> Simulator<M> {
    ...
    pub async fn run(&self, opp: &Opportunity, sized: &SizedTrade) -> SimulationReport {
        ...
        if passed {
            let (amt, gas, profit) = self.deep_simulate(sized, &mut trace).await;
            predicted_amount_out = amt;
            estimated_gas_units = gas;
            estimated_profit_after_gas_usd = profit;
    ...
    async fn check_rpc_health(&self, trace: &mut Vec<(String, CheckStatus)>) -> (bool, Option<Block<TxHash>>) {
        let consensus = match self.rpc_manager.consensus_block_number().await {
            Some(pair) => {
                trace.push(("rpc consensus".into(), CheckStatus::Pass(format!("blocks {}/{}", pair.a, pair.b))));
                true
            }
            None => {
                trace.push(("rpc consensus".into(), CheckStatus::Fail("disagree".into())));
                false
            }
        };
    ...
    async fn deep_simulate(
        ...
        let call_ok = match call_result {
            Ok(Ok(())) => {
                trace.push(("eth_call revert check".into(), CheckStatus::Pass("would not revert".into())));
                true
            }
            Ok(Err(e)) => {
                let decoded = format!("{e:?}");
                trace.push(("eth_call revert check".into(), CheckStatus::Fail(format!("would revert: {decoded}"))));
                false
            }
            Err(_) => {
                trace.push(("eth_call revert check".into(), CheckStatus::Fail("timeout".into())));
                false
            }
        };
